...
#엔코더불러오기
lb = LabelEncoder()#괄고잊지말긔
lb.fit(train_set['Sex'])
lb.fit(test_set['Sex'])
#클래스 학습
print(lb.classes_)
#라벨인코더가 학습한 고유값 출력 ['female' 'male']
train_set['Sex'] = lb.transform(train_set['Sex'])
test_set['Sex'] = lb.transform(test_set['Sex'])
print(train_set['Sex'])
print(test_set['Sex'])
...
#입력된 값 0,1 확인
#to categorical? get dummie? onehotencoder
#ocvtype에서 y연습했지만 여기선 x
#숫자열로 바뀌었으므로 칼럼은 나눠야하나? 결국엔 성능문제
# 0      1
# 1      0
# 2      0
# 3      0
...
#일단 임포트
#라벨인코딩은 문자-> 숫자로 바꿔주기만
#원핫은 문자->숫자 고유값별 칼럼을 나눠주므로 필요에 따라 달리쓰는 듯
#라벨인코딩 주석처리
...
# # reshape함수는 .values 에 있음
# #핏트한 분류가 먼지 프린트하는 건 못찾음
# #트랜스폼 재정의해야 하는듯?
# ohe.transform(train_set['Sex'].values.reshape(-1,1))
# print(ohe.categories_)
# pd.DataFrame(train_set['Sex'], columns=[])
# #아마도 트레인셋의 섹스를 판다스데이터프레임으로 정의하는데 이때 컬럼은 ohe에 학습된 결과로 시작열의 인덱스는 0이다?
# #fit와 마찬가지로 transform에서도 .values.reshape(-1,1)을 붙여줘야 넘어감
# #라벨엔코딩할때 reshape을 해주든가 여기서 해주든가 특정인수로 정의한다음에 그걸 reshape해주든가 
# #매트릭스 오류가 떠서 인코더 불러오기에 sparse=False 를 추가함 지금 필요한건 matrix가 아니고 array이기 때문에
# # print(type(ohe.categories_))
# #sex가 0.0 1.0으로 나눠진것 같음
# #toarray재정의하지말고 transform할때 붙여버림
# print(train_set)
#이미 트레인셋 섹스값 변환되었음
...
#섹스와 엠바크드 둘다 고유값이 많지 않고 카테고리별 위계관계가 있다고 보여지진 않기에 원핫인코딩만 쓴다
...
x = train_set.drop(['PassengerId', 'Survived', 'Name', 'Ticket', 'Cabin','Embarked'], axis=1)
y = train_set['Survived']
test_set = test_set.drop(['PassengerId', 'Name', 'Ticket', 'Cabin','Embarked'], axis=1)
print(x.shape)
print(x.head())
