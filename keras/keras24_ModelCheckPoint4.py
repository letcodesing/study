...
ES = EarlyStopping(monitor='val_loss', patience=40, mode='min', verbose=2, restore_best_weights=True)
MC = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1, save_best_only=True,
                     filepath=''.join([filepath,"_", df, '_', filename])
                     #""갖고오겠다 {[filepath 루트에 k24부터 현재날짜_시간_순서 파일네임                     
                     )

# start_time = p.time()
# #훈련을 인스턴스 하나로 줄여주기
h = model.fit(x_train, y_train, epochs=1000, batch_size=10,
...
# r2 -0.36892210788761437
# loss [36.80808639526367, 0.0, 4.805550575256348]
# =====================================로드출력===========================
# 4/4 [==============================] - 0s 5ms/step - loss: 30.5801 - accuracy: 0.0000e+00 - mae: 4.5764
# r22 -0.36892210788761437
# loss2 [30.580141067504883, 0.0, 4.5764312744140625]
# =====================================체크포인트출력===========================
# 4/4 [==============================] - 0s 0s/step - loss: 30.5801 - accuracy: 0.0000e+00 - mae: 4.5764
# r23 -0.1372998715393512
# loss3 [30.580141067504883, 0.0, 4.5764312744140625]
...
# r2 -0.06182143820446173
# loss [28.550643920898438, 0.0, 4.449565410614014]
#가중치가 저장된다면 r2가 같은값
# r2 -0.06182143820446173
# loss [28.550643920898438, 0.0, 4.449565410614014]
