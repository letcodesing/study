...
filename = '{epoch:04d} {val_loss:.41}'
filpath = './_ModelCheckPoint/k25/k11/'
mcp = ModelCheckpoint(filepath=([filpath,date,' ',filename]), monitor='val_loss', mode='auto', save_best_only=True)
# model = Sequential()
...
model.fit(x_train, y_train, epochs = 22, batch_size=200, verbose=1, callbacks=[mcp ])
end_time = time.time()
