...
y1_test, y2_train, y2_test = train_test_split(x1,y1,y2, train_size=0.7,random_state = 23
                                                        )
...
model = Model(inputs=input1, outputs=[output3, output31])
model.summary()


#3. 컴파일 훈련
model.compile(loss='mse', optimizer='adam')
model.fit(x1_train,[y1_train,y2_train], epochs=100, batch_size=2000)

#4.평가 예측
loss = model.evaluate(x1_test,[y1_test,y2_test])
# loss2 = model.evaluate(x1_test],y1_test)
